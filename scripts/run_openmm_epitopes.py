...
# Load the PDB file
fixer = PDBFixer(filename='../data/6Y97_epitopes_plus.pdb')
...
# Save the solvated structure
with open('../data/6y97_epitopes_plus_solvated.pdb', 'w') as f:
...
# Get ids of all atoms in the first and last residues in each chain
...
for chain in topology.chains():
    ...
    if len(residues) > 0:
        ...
        for atom in last_residue.atoms():
            first_last_atom_indices.append(atom.index)
# add massive harmonic restraints to these atoms
end_force = openmm.CustomExternalForce('0.5*k*(x^2 + y^2 + z^2)')
end_force.addPerParticleParameter('k')
k = 1e10 * unit.kilojoules_per_mole / unit.nanometers**2
for i in range(system.getNumParticles()):
    if i not in first_last_atom_indices:
        end_force.addParticle(i, [k])
system.addForce(end_force)
