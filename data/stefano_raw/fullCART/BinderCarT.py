...
def create_binder_carT( seqA: str, groupA: list, 
                   seq_CT1: str, dataB: Union[str, int], seq_CT2: str ) -> ProgramNode:
    '''INPUT:
    ...
    sequence_CT1 = ConstantSequenceSegment( seq_CT1 )
    sequence_CT2 = ConstantSequenceSegment( seq_CT2 )
    ...
    NCT1 = len( sequence_CT1.get() )
    NCT2 = len( sequence_CT2.get() )
    ...
    first_res_CT1_ID = first_res_dimer2_ID + MULTIMER_RESIDUE_INDEX_SKIP_LENGTH + NA2
    first_res_B_ID = first_res_CT1_ID + NCT1
    first_res_CT2_ID = first_res_B_ID + NB
    ...
    group_CT1 = range( first_res_CT1_ID, first_res_CT1_ID + NCT1 )
    groupB = range( first_res_B_ID, first_res_B_ID + NB )
    group_CT2 = range( first_res_CT2_ID, first_res_CT2_ID + NCT2 )
    ...
    return ProgramNode(
            ...
            EnergyLocalPLDDT(  group = group_CT1, chain_index = 2, verbose = False ), #Minimise error on groupB / binder region
            EnergyLocalPLDDT(  group = group_CT2, chain_index = 2, verbose = False ), #Minimise error on groupB / binder region
            ...
            root = True,
            children=[ ProgramNode( children = [ 
                                                  ProgramNode( sequence_segment = sequence_A1 ),
                                                  ProgramNode( sequence_segment = sequence_A2 ),
                                                  ],
                                    children_are_different_chains=True ),
                       ProgramNode( children = [ 
                                                  ProgramNode( sequence_segment = sequence_CT1 ),
                                                  ProgramNode( sequence_segment = sequence_B ),
                                                  ProgramNode( sequence_segment = sequence_CT2 ),
                                                ],
